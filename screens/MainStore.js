...
export default function MainStore({ staff, onLogout }) {
  ...
  const updateStatus = async (orderId, status) => {
    ...
    try {
      ...
      // Refresh stats when packing/collecting
      if (status === 'packing' || status === 'collected') {
        fetchStaffStats();
      }
    } catch {
  ...
  const handleQRScanned = async (orderId) => {
    ...
    if (order.status === 'collected') {
      Alert.alert('✅ Already Done',
        `${orderId} was already collected!`); return;
    }
    Alert.alert(
      '📱 QR Scanned!',
  ...
  const approveMechanic = async (id, approve) => {
    ...
    try {
      ...
      Alert.alert(
        ...
        approve
          ? 'Mechanic gets 5% discount!'
          : 'Registration rejected.'
      );
    } catch { Alert.alert('❌ Error', 'Could not update mechanic'); }
  };
    ...
    if (pinStep === 'current') {
      ...
      if (entered === newPin) {
        try {
          ...
          Alert.alert('⚠️ Saved Locally', `New PIN: ${newPin}`);
        }
  ...
  const saveProfile = async () => {
    if (!editName.trim()) {
      Alert.alert('❌', 'Name cannot be empty!'); return;
    }
    setSavingProfile(true);
    try {
      const r = await fetch(
        `${API_URL}/staff/${staff?.id}/profile`,
        {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
  ...
  const savePrice = async (productId) => {
    ...
    if (isNaN(mrp) || isNaN(selling)) {
      Alert.alert('❌', 'Enter valid prices'); return;
    }
    if (selling > mrp) {
      Alert.alert('❌', 'Selling price cannot exceed MRP!');
      return;
    }
    try {
    ...
    <SafeAreaView style={s.container}>
                ...
                <View style={[s.stockCard,
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() => Alert.alert(
                      ...
                      [
                        { text: '+1', onPress: () =>
                          updateStock(item.id, item.stock_qty+1) },
                        { text: '+5', onPress: () =>
                          updateStock(item.id, item.stock_qty+5) },
                        { text: '+10', onPress: () =>
                          updateStock(item.id, item.stock_qty+10) },
                        { text: 'Cancel', style: 'cancel' }
                      ]
                    )}
                  >
          ...
          >
            ...
            <View style={s.statsRow}>
              <StatCard icon="📦" label="Orders"
                value={summary?.total_orders||0} color="#4F6EF7" />
              <StatCard icon="💵" label="Cash"
                value={`₹${(summary?.payment_breakdown?.cash||0)
                  .toFixed(0)}`} color={G} />
              <StatCard icon="⏳" label="Pending"
                value={`₹${(summary?.payment_breakdown?.pending||0)
                  .toFixed(0)}`} color="#F59E0B" />
            </View>
            ...
            {isOwner && (
              <>
                <View style={s.reportCard}>
                  <Text style={s.reportCardTitle}>🔧 Mechanics</Text>
                  {mechanics.length === 0 ? (
                    <Text style={s.emptyText}>
                    ...
                    <View key={i} style={s.mechCard}>
                      ...
                      <View style={[s.mechStatus,
                        ...
                        <Text style={[s.mechStatusText,
                          {
                            color:m.status==='approved'?G
                              :m.status==='pending'?'#F59E0B'
                              :'#EF4444'
                          }]}>
                          {m.status}
                        </Text>
        ...
        {tab === 'profile' && (
          <ScrollView contentContainerStyle={{ padding: 16 }}>
            ...
            <View style={s.statsCardProfile}>
              <Text style={s.statsCardTitle}>
                📊 My Stats This Month
              </Text>
              <View style={s.statsGridProfile}>
                <View style={[s.statItemProfile,
                  { borderColor: 'rgba(245,158,11,0.3)' }]}>
                  <Text style={s.statItemIcon}>📦</Text>
                  <Text style={[s.statItemValue,
                    { color: '#F59E0B' }]}>
                    {staffStats?.packed || 0}
                  </Text>
                  <Text style={s.statItemLabel}>Packed</Text>
                </View>
                <View style={[s.statItemProfile,
                  { borderColor: 'rgba(34,197,94,0.3)' }]}>
                  <Text style={s.statItemIcon}>🏁</Text>
                  <Text style={[s.statItemValue, { color: G }]}>
                    {staffStats?.collected || 0}
                  </Text>
                  <Text style={s.statItemLabel}>Completed</Text>
                </View>
                <View style={[s.statItemProfile,
                  { borderColor: 'rgba(255,193,7,0.3)' }]}>
                  <Text style={s.statItemIcon}>💰</Text>
                  <Text style={[s.statItemValue,
                    { color: '#FFC107', fontSize: 15 }]}>
                    ₹{((staffStats?.revenue||0)/1000).toFixed(1)}k
                  </Text>
                  <Text style={s.statItemLabel}>Revenue</Text>
                </View>
              </View>
              <TouchableOpacity
                style={s.refreshStatsBtn}
                onPress={fetchStaffStats}
              >
                <Text style={s.refreshStatsBtnText}>
                  🔄 Refresh Stats
                </Text>
              </TouchableOpacity>
            </View>

            {/* PRICE MANAGER — owner + senior only */}
              ...
              <View style={s.detailCard}>
                ...
                {orderItems.length === 0 ? (
                  ...
                  <View key={i} style={[s.itemRow,
                    i===orderItems.length-1&&{borderBottomWidth:0}]}>
                    <View style={{ flex: 1 }}>
        ...
        <SafeAreaView style={s.container}>
          ...
          <ScrollView contentContainerStyle={{ padding: 20 }}>
            ...
            <View style={s.editInfoBox}>
              <Text style={s.editInfoText}>
                ℹ️ Your name appears on orders you process
              </Text>
              <Text style={s.editInfoText}>
                📱 Phone used for store communications
              </Text>
            </View>
            ...
            <View style={s.staffIdBox}>
              <Text style={s.staffIdText}>
                Staff ID: #{staff?.id}
              </Text>
              <Text style={s.staffIdNote}>Cannot be changed</Text>
            </View>
          </ScrollView>
        </SafeAreaView>
      </Modal>
      ...
      >
        <SafeAreaView style={s.container}>
          ...
          <View style={s.priceSearchBox}>
            <Text style={{ fontSize: 16 }}>🔍</Text>
            <TextInput
              style={s.priceSearchInput}
              placeholder="Search products..."
              placeholderTextColor="rgba(255,255,255,0.25)"
              value={priceSearch}
            ...
            {priceSearch.length > 0 && (
              <TouchableOpacity onPress={() => setPriceSearch('')}>
                <Text style={{
                ...
                {editingPriceId !== item.id ? (
                  <View style={s.priceDisplay}>
                    <View style={s.priceItem}>
                      <Text style={s.priceItemLabel}>MRP</Text>
                      <Text style={s.priceItemValue}>
                        ₹{item.mrp}
                      </Text>
                    </View>
                    <View style={s.priceDividerV} />
                    <View style={s.priceItem}>
                      <Text style={s.priceItemLabel}>Selling</Text>
                      <Text style={[s.priceItemValue,
                        {color:'#FFC107'}]}>
                        ₹{item.selling_price}
                      </Text>
                    ...
                    <View style={s.priceDividerV} />
                    <View style={s.priceItem}>
                      <Text style={s.priceItemLabel}>Margin</Text>
                      <Text style={[s.priceItemValue,
                        {color:G,fontSize:13}]}>
        ...
        <SafeAreaView style={s.container}>
          ...
          <View style={s.stepRow}>
            {['Current','New','Confirm'].map((st, i) => {
              const idx = pinStep==='current'?0
                :pinStep==='new'?1:2;
              return (
                <View key={i} style={s.stepItem}>
                  <View style={[s.stepCircle,
                    i<=idx && s.stepCircleActive]}>
          ...
          <View style={s.pinBody}>
            ...
            <Text style={s.pinTitleTe}>
              {pinStep==='current'?'ప్రస్తుత PIN నమోదు చేయండి'
               :pinStep==='new'?'కొత్త PIN నమోదు చేయండి'
...
const s = StyleSheet.create({
  ...
  tapHint: {
    fontSize: 10, color: 'rgba(34,197,94,0.4)', marginLeft: 'auto',
  },
  scannerBox: {
  ...
  toggleBtnText: {
    fontSize: 11, color: 'rgba(255,255,255,0.4)', fontWeight: 'bold',
  },
  ...
  avatarRing: {
    ...
    borderColor: G, alignItems: 'center',
  ...
  profileRole: {
    fontSize: 12, color: G, letterSpacing: 2, marginBottom: 4,
  },
  ...
  statsCardTitle: {
    fontSize: 14, fontWeight: 'bold', color: '#fff', marginBottom: 14,
  },
  statsGridProfile: { flexDirection: 'row', gap: 8, marginBottom: 12 },
  statItemProfile: {
    flex: 1, backgroundColor: 'rgba(34,197,94,0.05)',
    borderRadius: 12, padding: 12, alignItems: 'center',
    gap: 4, borderWidth: 1,
  },
  statItemIcon: { fontSize: 22 },
  statItemValue: { fontSize: 22, fontWeight: 'bold', color: G },
  statItemLabel: {
  ...
  refreshStatsBtnText: {
    color: G, fontSize: 13, fontWeight: 'bold',
  },
  clockCard: {
    backgroundColor: '#0D1A0D', borderRadius: 16, padding: 16,
    flexDirection: 'row', alignItems: 'center', gap: 14,
  ...
  logoutText: {
    color: '#EF4444', fontWeight: 'bold', fontSize: 15,
  },
  ...
  itemSku: {
    fontSize: 10, color: 'rgba(34,197,94,0.4)', letterSpacing: 1,
  },
  ...
  payBtn: {
    ...
    borderColor: 'rgba(34,197,94,0.15)', alignItems: 'center',
  },
  payBtnText: {
  ...
  editAvatarRole: {
    fontSize: 12, color: G, letterSpacing: 2, fontWeight: 'bold',
  },
  editLabel: {
  ...
  editPhonePrefix: {
    fontSize: 16, color: 'rgba(255,255,255,0.4)', fontWeight: 'bold',
  },
  editPhoneInput: { flex: 1, color: '#fff', fontSize: 16 },
  editInfoBox: {
    backgroundColor: 'rgba(34,197,94,0.05)', borderRadius: 12,
    padding: 12, marginBottom: 20, gap: 6, borderWidth: 1,
    borderColor: 'rgba(34,197,94,0.1)',
  },
  editInfoText: { fontSize: 12, color: 'rgba(255,255,255,0.4)' },
  saveProfileBtn: {
  ...
  staffIdBox: { alignItems: 'center', gap: 4 },
  staffIdText: { fontSize: 13, color: 'rgba(255,255,255,0.3)' },
  staffIdNote: { fontSize: 11, color: 'rgba(255,255,255,0.15)' },
  ...
  priceSearchInput: { flex: 1, color: '#fff', fontSize: 14 },
  priceCard: {
    backgroundColor: '#0D1A0D', borderRadius: 14, padding: 14,
    marginBottom: 10, borderWidth: 1,
    borderColor: 'rgba(34,197,94,0.15)',
  ...
  priceProductSku: {
    fontSize: 10, color: 'rgba(34,197,94,0.5)', letterSpacing: 1,
  },
  editPriceBtn: {
    backgroundColor: 'rgba(34,197,94,0.1)', borderRadius: 10,
  ...
  priceDividerV: {
    width: 1, height: 30, backgroundColor: 'rgba(34,197,94,0.15)',
  },
  priceEditForm: { gap: 10 },
  priceEditRow: { flexDirection: 'row', gap: 10 },
  priceEditField: { flex: 1 },
  priceEditLabel: {
    fontSize: 11, color: 'rgba(255,255,255,0.4)', marginBottom: 6,
  },
  priceEditInput: {
    backgroundColor: 'rgba(34,197,94,0.05)', borderRadius: 10,
    padding: 12, color: '#fff', fontSize: 16, borderWidth: 1,
    borderColor: 'rgba(34,197,94,0.2)',
  },
  savePriceBtn: {
  ...
  pinTitleTe: {
    fontSize: 12, color: 'rgba(34,197,94,0.4)', marginBottom: 24,
  },
